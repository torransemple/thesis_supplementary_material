---
title: "Synthetic data: fuel poverty experiments"
...
### Fuel poverty measurement experiments on synthetic data
```{r}

# This notebook contains fuel poverty classifications performed on the synthetic data (i.e., 'assigned_synth_income_energy_costs.csv'; i.e., the output csv from 'notts_LSOA_synth_income_distributions.Rmd')

# Load data (assigned_synth_income_energy_costs.csv)
synth <- read.csv("/Users/psxts6/Desktop/FP_income_PoC/code_and_data/assigned_synth_income_energy_costs.csv", header = TRUE, sep = ",")


# clean: convert obscure 'INVALID!' observations to NAs and remove
library(dplyr)
synth <- synth %>%
  mutate(
...
# Create EPC A-C versus D-G column for later experiments
synth <- synth %>%
...
# Merge with IMD for later visuals and create IMD quintile column
...
### 10% Fuel Poverty (i.e. Fuel Poverty Ratio): including basic 10%, 20% (i.e., 'extreme' fuel poverty) and five-level 10%
```{r}

# Create a new column 'fuel_poverty_ratio' 
synth <- synth %>%
...
# Extend further to five-level 10% (where 'not fuel poor' = <8%; 'marginal fuel poverty' = 8-10%; 'fuel poverty' = 10-13%; 'severe fuel poverty' = 13-20% and 'extreme fuel poverty' = >20%)
...
# Create some summary stats for 10/20/five-level 10 per LSOA (Note: have lost 18 LSOAs due to incompatibility between epc and synth (i.e., GDHI) LSOAs)
...
## Visualisation of five-level 10% fuel poverty
```{r, warning=FALSE}

# Visualise five-level 10%
...
# Calculate proportions and set the factor levels
...
# Calculate LIHC fuel poverty among synthetic Nottingham homes
synth <- synth %>%
  mutate(LIHC = ifelse(energy_costs > 2252 & income < 18840 + energy_costs, 1, 0))
...
## LILEE* (i.e. SAP instead of FPEER)
```{r}

# Calculate LILEE* (i.e., SAP version of LILEE)
...
## Create subset of LSOAs that are of particular interest in following chunks
```{r}

# create synth_lsoa_subset 
...
## Further viusals on LSOA subset
```{r, warning=FALSE}
...
p1 <- ggplot(synth_lsoa_subset, aes(x = income, y = energy_costs)) +
  geom_point(data=LILC, size=3, colour= "#DDB0D9", alpha = 0.6) +
...
slope <- (0 - 100) / (25522 - 19258)

p3 <- ggplot(synth_lsoa_subset, aes(x = income, y = current_energy_efficiency)) +
  geom_point(data = LIHEE, size = 3, colour = "#DDB0D9", alpha = 0.6) +
...
## LIHC and LILEE crosstabulations with 10% fuel poverty
```{r}
...
## Further five-level fuel poverty visualisations on LSOA susbet
```{r, warning=FALSE}
...
# write.csv(x=summary_five_level_by_imd, file="five_fold_10_by_imd_quintile.csv")


# Check strange distribution of five-fold 10% among homes in IMD quintile 4
imd_quintile_4 <- synth %>%
  filter(imd_quintile == 4)

table(imd_quintile_4$lsoa_2011)
...
# The fuel poverty gap is "the amounts by which the assessed energy needs of fuel poor households exceed the threshold for reasonable costs"; i.e., among LIHC fuel poor homes, the fuel poverty gap = required energy costs - median energy costs (2.25k/yr)
...
## 10% and 8% ("precarity") fuel poverty gap
```{r, warning=FALSE}
...
# Create gap vs buffer subsets (for 8 and 10%) to make summary stats more intuitive
...
# 8% gap
eight_fp_gap <- synth %>%
  filter(`8%_fuel_poverty_gap_and_buffer` >= 0)

colnames(eight_fp_gap)[30] <- "eight_fp_gap"
cols_to_drop <- c(29) # drop 10% to avoid confusion 
eight_fp_gap <- eight_fp_gap[, -cols_to_drop]

# Calculate median 8% gap per LSOA and join LSOA summary table
median_eight_fp_gap <- eight_fp_gap %>%
  group_by(lsoa_2011) %>%
  summarise(median_eight_fp_gap = median(eight_fp_gap, na.rm = TRUE))
LSOA_summary_fp <- left_join(LSOA_summary_fp, median_eight_fp_gap, by = "lsoa_2011") # Join




# 8% buffer
eight_fp_buffer <- synth %>%
  filter(`8%_fuel_poverty_gap_and_buffer` < 0)

colnames(eight_fp_buffer)[29] <- "eight_fp_buffer"
cols_to_drop <- c(28) # drop 10% to avoid confusion 
eight_fp_buffer <- eight_fp_buffer[, -cols_to_drop]

# Calculate median 8% buffer per LSOA and join LSOA summary table
median_eight_fp_buffer <- eight_fp_buffer %>%
  group_by(lsoa_2011) %>%
  summarise(median_eight_fp_buffer = median(eight_fp_buffer, na.rm = TRUE))
LSOA_summary_fp <- left_join(LSOA_summary_fp, median_eight_fp_buffer, by = "lsoa_2011") # Join
...
# Summary stats: 8% fuel poverty gap and buffer
eight_gap_summary <- summary(eight_fp_gap$eight_fp_gap)
eight_buffer_summary <- summary(eight_fp_buffer$eight_fp_buffer)
eight_aggregate_gap <- sum(eight_fp_gap$eight_fp_gap, na.rm = TRUE)
eight_aggregate_buffer <- sum(eight_fp_buffer$eight_fp_buffer, na.rm = TRUE)
...
assign_composition_weighted_area <- function(normalized_area, proportions, types) {
  # Define how area influences probability
...
## MIS-based Fuel Poverty: including MIS + 10% and MIS + five-level 10% + fuel poverty gap/buffer
```{r}
...
## The closeness of LILEE* (my estimates) versus LILEE (DESNZ, 2023-based on 2021 data)
```{r, warning=FALSE}

# Load LILEE data
desnz_lilee <- read.csv("/Users/psxts6/Desktop/FP_income_PoC/code_and_data/desnz_lilee_2023_21_data.csv", header = TRUE, sep = ",")
...
print(mean_income_by_yellow_cluster)


```


## Data Export: LSOA Summaries for Maps & Synth (plus FP estimates)
```{r}
